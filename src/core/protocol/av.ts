...
function getSampleRate(bitop: Bitop, info: any): number {
...
function readH264SpecificConfig(avcSequenceHeader: Buffer): any {
    let info: any = {};
...
function HEVCParsePtl(bitop: Bitop, hevc: any, max_sub_layers_minus1: number): any {
    let general_ptl: any = {};
...
function HEVCParseSPS(SPS: Buffer, hevc: any): any {
    let psps: any = {};
    let NumBytesInNALunit = SPS.length;
    let rbsp_array: number[] = [];
    ...
    if (psps.conformance_window_flag) {
        let vert_mult = 1 + (
            psps.chroma_format_idc < 2 ? 1 : 0
        );
        let horiz_mult = 1 + (
            psps.chroma_format_idc < 3 ? 1 : 0
        );
...
function readHEVCSpecificConfig(hevcSequenceHeader: Buffer): any {
    let info: any = {};
    info.width = 0;
    info.height = 0;
    info.profile = 0;
    ...
    do {
        ...
        hevc.general_profile_space = (
            hevcSequenceHeader[1] >> 6
        ) & 0x03;
        hevc.general_tier_flag = (
            hevcSequenceHeader[1] >> 5
        ) & 0x01;
        ...
        hevc.min_spatial_segmentation_idc = (
            (
                hevcSequenceHeader[13] & 0x0F
            ) << 8
        ) | hevcSequenceHeader[14];
        ...
        hevc.avgFrameRate = (
            hevcSequenceHeader[19] << 8
        ) | hevcSequenceHeader[20];
        hevc.constantFrameRate = (
            hevcSequenceHeader[21] >> 6
        ) & 0x03;
        hevc.numTemporalLayers = (
            hevcSequenceHeader[21] >> 3
        ) & 0x07;
...
function readAV1SpecificConfig(av1SequenceHeader: Buffer): any {
    let info: any = {};
    info.width = 0;
    info.height = 0;
    info.profile = 0;
