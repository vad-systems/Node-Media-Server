...
function generateS1(messageFormat: MessageFormat) {
...
function generateS2(messageFormat: MessageFormat, clientsig: Buffer) {
...
class RtmpPacket {
    private _header: RtmpPacketHeader;
    ...
    public get header(): RtmpPacketHeader {
        return this._header;
    }

    public set header(value: RtmpPacketHeader) {
        this._header = value;
    }
    ...
    public set payload(value: Buffer) {
        this._payload = value;
    }
...
class Rtmp {
    private handshakePayload = Buffer.alloc(RTMP_HANDSHAKE_SIZE);
    private handshakeState = RtmpHandshakeState.UNINIT;
    private handshakeBytes = 0;

    private parserBuffer = Buffer.alloc(MAX_CHUNK_HEADER);
    private parserState = RtmpParserState.INIT;
    private parserBytes = 0;
    private parserBasicBytes = 0;
    private parserPacket: RtmpPacket | null = null;
    private inPackets: Map<number, RtmpPacket> = new Map();
    ...
    private streams = 0;
    private streamId = 0;
    private streamQuery: ParsedUrlQuery;
    ...
    static chunkMessageHeaderCreate = (header: RtmpPacketHeader) => {
    ...
    sendDataMessage(sid: number, opt: any) {
        let packet = new RtmpPacket();
        packet.header.fmt = RtmpChunk.TYPE_0;
