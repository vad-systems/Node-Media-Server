...
class NodeFissionServer extends nms_server_1.NodeTaskServer {
    ...
    async run() {
        ...
        if (version === '' || parseInt(version.split('.')[0]) < 4) {
            ...
            this.logger.error('[Fission] Download the latest ffmpeg static program:', nms_core_1.NodeCoreUtils.getFFmpegUrl());
            return;
        }
    ...
    handleTaskMatching(session, app, name) {
        ...
        for (let task of this.config.fission.tasks) {
            if (!(0, nms_shared_1.checkSelectiveTask)(task, app, session.streamPath)) {
                this.logger.debug(`[Fission] pattern check failed: pattern=${task.pattern} srcId=${srcId} app=${app} streamPath=${session.streamPath}`);
                continue;
            }
            ...
            if (isExisting) {
                this.logger.debug(`[Fission] session still running: srcId=${srcId} streamPath=${session.streamPath}`);
                continue;
            }
            ...
            if (broadcast.publisher.isLocal() && nameSegments.length > 0 && !isNaN(parseInt(nameSegments[nameSegments.length - 1]))) {
                this.logger.debug(`[Fission] duplication check failed: pattern=${task.pattern} srcId=${srcId} app=${app} streamPath=${session.streamPath}`);
                continue;
            }
            ...
            this.logger.log(`[Fission] session start: srcId=${srcId} id=${id} streamPath=${sessionConf.streamPath} tasks=${taskConf.model.length}`);
            sess.on('end', (id) => {
                this.logger.log(`[Fission] session ended: srcId=${srcId} id=${id} streamPath=${sessionConf.streamPath} tasks=${taskConf.model.length}`);
                ...
                setTimeout(() => {
                    if (broadcast && broadcast.publisher) {
                        this.logger.log(`[Fission] session restart: srcId=${srcId} id=${id} streamPath=${sessionConf.streamPath} tasks=${taskConf.model.length}`);
                        this.handleTaskMatching(broadcast.publisher, app, name);
                    }
                }, 1000);
            });
            sess.start();
            this.logger.log(`[Fission] session started: srcId=${srcId} id=${id} streamPath=${sessionConf.streamPath} tasks=${taskConf.model.length}`);
        }
    }
    stop() {
        super.stop();
