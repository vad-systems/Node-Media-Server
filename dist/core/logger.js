...
let logType = types_js_1.LogType.NORMAL;
const setLogType = (type) => {
    if (Object.values(types_js_1.LogType).indexOf(type) === -1) {
        return;
    }
    logType = type;
};
...
const log = (...args) => {
    ...
    if (logType < types_js_1.LogType.NORMAL) {
        return;
    }
...
const error = (...args) => {
    ...
    if (logType < types_js_1.LogType.ERROR) {
        return;
    }
...
const debug = (...args) => {
    ...
    if (logType < types_js_1.LogType.DEBUG) {
        return;
    }
...
const ffdebug = (...args) => {
    ...
    if (logType < types_js_1.LogType.FFDEBUG) {
        return;
    }
